...
// WORKING HERE
    ...
    // ledcWriteNote(BUZZ_CHANNEL, NOTE_MAX, octave);
    ledcWriteTone(BUZZ_CHANNEL, 4000);
    if((int)(t/5)%2 == 0){
      target_omega = 0;
      target_v= 0;
    }
    else if(adc1_buf[3]<zalue){//middle sensor 7 is over white line, go straight
      target_omega = 0;
      target_v= .02;
      target_theta = target_theta + target_omega *dt;
    }
    else if(adc1_buf[3]>zalue && adc2_buf[3]>zalue){ // middle is DARK, sens8 is dark, needs to turn right
      target_v=0;
    ...
    else if(adc1_buf[3]>zalue && (adc2_buf[3]<zalue || adc1_buf[4]<zalue)){ //if middle is dark, and 8 and 9 is light, turn left
      target_v = 0;
       //Serial.println("NOW HERE!");
      target_omega = .15; //turn left
      target_theta = target_theta + target_omega * dt;
    }
    else if(adc1_buf[3]>zalue && (adc2_buf[2]<zalue || adc1_buf[2]<zalue)){ //if middle is dark, and 6 and 5 is light, turn right
      target_v = 0;
       //Serial.println("NOW HERE!");
      target_omega = -.15; //turn right
      target_theta = target_theta + target_omega * dt;
    }
    else if(adc1_buf[3]>zalue && (adc1_buf[0]<zalue|| adc2_buf[0]<zalue)) { //sensor 1 and 2 is over white, middle is dark
      target_v = 0; //maybe make 0
      target_omega = -.5; //turn right
      target_theta = target_theta + target_omega * dt;
    }
    else if(adc1_buf[3]>zalue && (adc1_buf[5]<zalue || adc2_buf[4]<zalue)) { //middle dark, 10 or 11 light, turn left
      target_v = 0; //maybe make 0
